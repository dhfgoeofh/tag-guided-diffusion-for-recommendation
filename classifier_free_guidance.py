...
class MLP(nn.Moudle):
    ...
    """
    def __init__ (self, in_dims, out_dims, time_emb_dim, tag_emb_dim, norm=False, dropout=0.5):
        ...
        self.tag_embedding = nn.Sequential(
            nn.Linear(tag_emb_dim, in_dims),
            nn.GELU(),
            nn.Linear(in_dims, in_dims),
        )

        if self.time_type == "cat":
            in_dims_temp = [self.in_dims[0] + self.time_emb_dim + self.tag_emb_dim] + self.in_dims[1:]
        else:
