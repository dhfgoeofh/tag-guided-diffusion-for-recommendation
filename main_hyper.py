...
def parse_args():
    ...
    parser.add_argument('--num_t_samples', type=str, default='[1, 2, 3]', help='number of time(t) samples for training') ###
    ...
    parser.add_argument('--batch_size', type=str, default='[200, 400, 600]')    ###
...
if __name__ == '__main__':
    ...
    for bs in eval(args.batch_size):
        ...
        for nt in eval(args.num_t_samples):
            for ts in eval(args.timesteps):
                print(f"batch_size: {bs}, num_t_sample: {nt}, timesteps: {ts}")
                ### model ###
                model = MLP(
